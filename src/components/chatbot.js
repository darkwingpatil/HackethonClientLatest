...
function Chatbot({props}) {
  ...
  var socket = io("https://seamlesslearning.onrender.com", { transports: ["websocket"] });
  let previousDays = 0;
  let currentDay = 0;
  useEffect(() => {
    socket.on("connect", () => {
  ...
  const get_user_chat_history = () => {
    fetch(`https://seamlesslearning.onrender.com/entireConversation?userHandle=${userHandle}`)
      .then((res) => res.json())
      .then(({ data }) => {
  ...
  const thumbupHandler = (index, userHandle, sessionId, courseId) => {
    if (thumbsup[`${index}${userHandle}${sessionId}${courseId}`]) {
      ...
      fetch("https://seamlesslearning.onrender.com/updatefeedback", {
        'method': "POST",
        'headers': {
          ...
          <div className="chatbox-header">
            ...
            <div className="chatbox-side-options">
              ...
              <div
                ...
                onClick={() => {
                  setIsOpen(false);
                  fetch(`https://seamlesslearning.onrender.com/oncloseaddtohistory?userHandle=${userHandle}&courseId=${courseId}`)
                    .then((res) => res.json())
                    .catch((err) => console.log(err))
                }}
              >
